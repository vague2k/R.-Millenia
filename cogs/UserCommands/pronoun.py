...
class PronounGroup(commands.GroupCog, group_name = "pronoun"):
    ...
    async def give_pronoun_role(self, interaction: discord.Interaction, pronouns: Literal["he/him", "she/her", "they/them", "it/its", "any pronouns"]):
        ...
        if role in member.roles: # type: ignore
            await interaction.response.send_message("It looks like you already have this pronoun as a role. If you would like to remove it, please use `m.pronoun remove`.", ephemeral = True)
    ...
    async def change_pronoun_role(self, interaction: discord.Interaction, remove: Literal["he/him", "she/her", "they/them", "it/its", "any pronouns"]):
        member = interaction.user
        ...
        if role not in member.roles: # type: ignore
            await interaction.response.send_message("It looks like this role was either never given or already removed. If you would like to add it, please use `m.pronoun give`.", ephemeral = True)

        else:
            await member.remove_roles(role) # type: ignore
            await interaction.response.send_message(f"The role you chose, **{role}** has been removed, Enjoy!", ephemeral = True)
